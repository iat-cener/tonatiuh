...
void SceneModel::SetMedium()
{

    if ( m_coinScene->getPart( "medium", false ) )
    {
    	SoNode* environmentNode =  m_coinScene->getPart( "medium", false );
    	InsertEnvironmentNode( *environmentNode );
    }
...
QVariant SceneModel::data( const QModelIndex& modelIndex, int role ) const
{
    ...
    if ( modelIndex.column() == 0 )
    {
        ...
        if (role == Qt::DecorationRole)
        {
			...
			else if( coinNode->getTypeId().isDerivedFrom( TMedium::getClassTypeId() ) )
			{
				TMedium* medium = static_cast< TMedium* >( coinNode );
				return QIcon( medium->GetIcon() );
			}
     	}
    }
    return QVariant();
...
/*!
 * Insert a environment node to the model. If the model has an other environment node, the previous node
 * will be deleted.
 */
void SceneModel::InsertEnvironmentNode( SoNode& environmentNode )
{
	if( ( m_instanceRoot->children.size() > 0 ) &&
				( m_instanceRoot->children[0]->GetNode()->getTypeId().isDerivedFrom( SoEnvironment::getClassTypeId() ) ) )
			m_instanceRoot->children.remove( 0 );

	SoNode* envNodeCopy = environmentNode.copy( true );
	m_coinScene->setPart( "medium", &environmentNode );

	m_coinRoot->insertChild( envNodeCopy, 0 );

	InstanceNode* instanceEnvironment = new InstanceNode( envNodeCopy );
	m_instanceRoot->InsertChild( 0, instanceEnvironment );
...
void SceneModel::InsertLightNode( TLightKit& coinLight )
{
	...
	if ( lightList->getNumChildren() > 0 )
		if( m_instanceRoot->children.size() > lightIndex ) m_instanceRoot->children.remove( lightIndex );
	...
	m_instanceRoot->InsertChild( lightIndex, instanceLight );
...
void SceneModel::RemoveEnvironmentNode( SoNode& environmentNode )
{
	if( m_instanceRoot->children[0]->GetNode()->getTypeId().isDerivedFrom( SoEnvironment::getClassTypeId() ) )
	{
		m_coinRoot->removeChild( 0 );
		m_instanceRoot->children.remove( 0 );
		m_coinScene->setPart( "medium", 0 );
...
void SceneModel::RemoveLightNode( TLightKit& coinLight )
{
    ...
    m_instanceRoot->children.remove( lightIndex );
