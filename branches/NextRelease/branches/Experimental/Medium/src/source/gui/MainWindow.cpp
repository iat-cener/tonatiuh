...
void MainWindow::SetupVRMLBackground()
{
	...
	//m_document->GetRoot()->insertChild( vrmlBackground, 1 );
    m_document->GetRoot()->addChild( vrmlBackground );
}
...
void MainWindow::SetupParametersView()
{
    ...
    connect ( parametersView, SIGNAL(valueModificated( const QStringList&, SoNode*, QString  ) ),
		                this, SLOT( parameterModified( const QStringList&, SoNode*, QString  ) ) );
...
void MainWindow::LoadTonatiuhPlugin( const QString& fileName )
{
    ...
    if ( plugin != 0)
    {
    	...
    	if( plugin->inherits( "TMediumFactory"  ) ) LoadMediumPlugin( plugin );
...
void MainWindow::LoadMediumPlugin( QObject* plugin )
{
	TMediumFactory* pTMediumFactory = qobject_cast<TMediumFactory* >( plugin );
	if( !pTMediumFactory ) tgf::SevereError( "MainWindow::LoadPlugins: Medium plug-in not recognized" );;
	m_TMediumFactoryList.push_back( pTMediumFactory );
	pTMediumFactory->CreateTMedium();
}
...
/**
 * Creates a new tool bar for /a pMaterialsMenu actions.
 */
...
/**
 * Creates a new tool bar for /a pTrackersMenu actions.
 */
...
/**
 * Creates an action for the /a pTMaterialFactory and adds to material insert menu and toolbar.
 */
...
/*!
 * Defines a atmospheric attenuation in for Tonatiuh ray tracer.
 */
void MainWindow::on_actionDefineMedium_triggered()
{
	DefineMediumDialog dialog( m_TMediumFactoryList );
	if( !dialog.exec() )	return;

	TMedium* medium = dialog.GetTMedium();
	if( !medium )	return;

	//CmdInsertMedium cmd;
	CmdInsertMedium* cmdInsertMedium = new CmdInsertMedium( medium, m_sceneModel, m_document->GetRoot() );
	cmdInsertMedium->setText( "Insert Medium node" );
	m_commandStack->push( cmdInsertMedium );
...
bool MainWindow::ReadyForRaytracing( InstanceNode*& rootSeparatorInstance, InstanceNode*& lightInstance, SoTransform*& lightTransform, TSunShape*& sunShape, TShape*& raycastingShape )
{
	...
	int rootSeparatorIndex = 1;
	if( m_document->GetRoot()->getChild( 0 )->getTypeId().isDerivedFrom( SoEnvironment::getClassTypeId() ) )
		rootSeparatorIndex = 2;

	rootSeparatorInstance = sceneInstance->children[rootSeparatorIndex];
...
void MainWindow::ShowRaysIn3DView()
{
	...
	if( ( m_document->GetRoot()->getNumChildren() > raysIndex ) && ( m_document->GetRoot()->getChild( raysIndex )->getName() == "Rays"  ) )
		m_document->GetRoot()->removeChild( raysIndex );
...
void MainWindow::on_actionRayTraceRun_triggered()
{
	...
	if( ReadyForRaytracing( rootSeparatorInstance, lightInstance, lightTransform, sunShape, raycastingSurface ) )
	{
		...
		TMedium* medium = 0;
		if( m_document->GetRoot()->getChild( 0 )->getTypeId().isDerivedFrom( SoEnvironment::getClassTypeId() ) )
			medium = static_cast< TMedium* > ( m_document->GetRoot()->getChild( 0 ) );
		...
		QMutex mutex;
		QFuture< TPhotonMap* > photonMap = QtConcurrent::mappedReduced( raysPerThread, RayTracer(  rootSeparatorInstance, raycastingSurface, sunShape, lightToWorld, medium, *m_rand, &mutex, m_photonMap ), createPhotonMap, QtConcurrent::UnorderedReduce );
...
void MainWindow::on_actionDisplay_rays_toggled()
{
	...
	{
	  	m_document->GetRoot()->insertChild( m_pRays, raysIndex );
	}
	else if( !actionDisplay_rays->isChecked() )
	{
		if ( m_pRays->getRefCount( ) > 0 )	m_document->GetRoot()->removeChild( raysIndex );
	}
}
...
void MainWindow::on_actionExport_PhotonMap_triggered()
{
	...
	else
	{
		...
		{
			QMessageBox::information( this, "Tonatiuh Error",
										"There are no photons to export\n", 1);
			return;
		}
...
{
	int vrmlBackgroundIndex = 0;
	if( ! m_document->GetRoot()->getChild( 0 )->getTypeId().isDerivedFrom( SoEnvironment::getClassTypeId() ) )
		vrmlBackgroundIndex = 1;

	SoVRMLBackground* vrmlBackground = static_cast< SoVRMLBackground* > ( m_document->GetRoot()->getChild( vrmlBackgroundIndex ) );
...
void MainWindow::selectionChanged( const QModelIndex& current, const QModelIndex& /*previous*/ )
{
    ...
    parametersView->ChangeParameters( selectedCoinNode );
...
bool MainWindow::StartOver( const QString& fileName )
{
	...
	if( m_document->GetRoot()->getChild( 0 )->getTypeId().isDerivedFrom( SoEnvironment::getClassTypeId() ) )
		m_document->GetRoot()->removeChild( 0 );

	actionDisplay_rays->setEnabled( false );
	actionDisplay_rays->setChecked( false );

	int raysIndex = m_document->GetRoot()->getNumChildren() - 1;
	if( m_document->GetRoot()->getChild( raysIndex )->getName() == "Rays"  )
			m_document->GetRoot()->removeChild( raysIndex );
 ...
 **/
void MainWindow::parameterModified( const QStringList& oldValueList, SoNode* coinNode, QString coinPart )
{
