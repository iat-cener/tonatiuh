 ...
 ***************************************************************************/
 ...
 const int RADIO = 120;
 const double ECLIPTIC = 23.5 *( tgc::Pi/180 );


 CelestialWidget::CelestialWidget( QWidget* parent )
 :QWidget( parent ), m_declination( 0 ), m_rightAscension( 0 )
 {
	 ...
	 m_rootNode->addChild( Sphere(RADIO) );
	 m_rootNode->addChild( CelestialEquator( RADIO ) );
 ...
 SoSeparator* CelestialWidget::CelestialEquator( int r ) const
 {
 ...
 SoSeparator* CelestialWidget::Declination()
 {
	 ...
	 for( int theta = 0; theta< 360; theta++ )
	 {
		 ...
		 declinationPoints[theta][0] = sin( m_rightAscension ) * cos( grad )* RADIO;
		 declinationPoints[theta][1] = sin( grad ) * RADIO;
		 declinationPoints[theta][2] = cos( m_rightAscension ) * cos( grad ) * RADIO ;
	 }
	 ...
	 //curveMaterial->transparency.setValue( 0.3f );
	 ...
	 for( int index = 1; index <= 90; index ++ )
	 {
		 ...
		 curvePoints[numPoints][0] = sin( m_rightAscension ) * cos( grad1 )* RADIO;
		 curvePoints[numPoints][1] = sin( grad1 ) * RADIO;
		 curvePoints[numPoints][2] = cos( m_rightAscension ) * cos( grad1 ) * RADIO ;
		 indexes[ ( ( index -1 ) * 4 ) +1 ] = numPoints;
		 numPoints++;

		 double grad2 = ( m_declination / 360 ) * 4 * index;
		 curvePoints[numPoints][0] = sin( m_rightAscension ) * cos( grad1 )* RADIO;
		 curvePoints[numPoints][1] = sin( grad2 ) * RADIO;
		 curvePoints[numPoints][2] = cos( m_rightAscension ) * cos( grad2 ) * RADIO ;
 ...
 SoSeparator* CelestialWidget::Ecliptic() const
 {
	 ...
	 for( int fi = 0; fi<360; fi++ )
	 {
		 p[fi][0] = cos( ECLIPTIC ) * sin( fi * (tgc::Pi / 180) ) * RADIO;
		 p[fi][1] = sin( ECLIPTIC ) * sin( fi * (tgc::Pi / 180) ) * RADIO;
		 p[fi][2] = -cos( fi * (tgc::Pi / 180) ) * RADIO;
 ...
 SoSeparator* CelestialWidget::Ejes() const
 {
	 ...
	 float p[2][3]={ {0.0, -RADIO-40, 0.0}, {0.0, RADIO+40, 0.0} };
	 ...
	 norteTransf->translation.setValue( SbVec3f( 0.0, RADIO+30, 0.0 ) );
	 ...
	 southTransf->translation.setValue( SbVec3f( 0.0, -RADIO-30, 0.0 ) );
 ...
 Trace( "CelestialWidget::Points", false );
	 ...
	 ariesTransform->translation.setValue( 0.0f, 0.0f, RADIO );
	 ...
	 ariesTextTransf->translation.setValue( SbVec3f( 0.0, 0.0, RADIO+30 ) );
	 ...
	 libraTransform->translation.setValue( 0.0f, 0.0f, -RADIO );
	 ...
	 libraTextTransf->translation.setValue( SbVec3f( 0.0, 0.0, -RADIO-30 ) );
 ...
 SoSeparator* CelestialWidget::RightAscension()
 {
	 ...
	 for( int fi = 0; fi < 360; fi++ )
	 {
		 ...
		 ascensionPoints[fi][0] = sin( grad )* RADIO ;
		 ascensionPoints[fi][1] = 0.0;
		 ascensionPoints[fi][2] = cos( grad ) * RADIO ;
	 ...
	 for( int index = 1; index <= 90; index ++ )
	 {
		 ...
		 curvePoints[numPoints][0] = sin( grad1 )* RADIO ;
		 curvePoints[numPoints][1] = 0.0;
		 curvePoints[numPoints][2] = cos( grad1 ) * RADIO ;
		 indexes[ ( ( index -1 ) * 4 ) +1 ] = numPoints;
		 numPoints++;

		 double grad2 = ( m_rightAscension / 360 ) * 4 * index;
		 curvePoints[numPoints][0] = sin( grad2 )* RADIO ;
		 curvePoints[numPoints][1] = 0.0;
		 curvePoints[numPoints][2] = cos( grad2 ) * RADIO ;
		 indexes[ ( ( index -1 ) * 4 ) +2 ] = numPoints;
		 numPoints++;
 ...
 SoSeparator* CelestialWidget::Sphere( int r ) const
 {
 ...
 SoSeparator* CelestialWidget::Star()
 {
	 ...
	 starTransform->translation.setValue( cos( m_declination )* sin( m_rightAscension ) * RADIO, sin( m_declination ) * RADIO, cos( m_declination )* cos( m_rightAscension) * RADIO ) ;
