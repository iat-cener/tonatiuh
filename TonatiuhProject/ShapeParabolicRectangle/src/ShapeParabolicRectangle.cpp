...
ShapeParabolicRectangle::ShapeParabolicRectangle()
{
	...
	SO_NODE_ADD_FIELD(m_focusxz, (1.0));
	SO_NODE_ADD_FIELD(m_focusyz, (1.0));
	SO_NODE_ADD_FIELD(m_xlength, (10.0));
	SO_NODE_ADD_FIELD(m_ylength, (10.0));
}
...
//We use as local coordinate system the one centered in the 1st parabolic cylinder (XZ) vertex.	
	
	//Calculation of the z0 coordinate for the 2nd parabolic cylinder (YZ)
	double z0 = xmax*xmax/(4*m_focusxz.getValue())-ymax*ymax/(4*m_focusyz.getValue());

	double zmax = xmax*xmax/(4*m_focusxz.getValue());
	double zmin = (z0 >= 0.0) ? z0 : 0.0;
...
bool ShapeParabolicRectangle::Intersect(const Ray& objectRay, double *tHit, DifferentialGeometry *dg) const 
{
	...
	// Compute quadratic coefficients for the 1st parabolic cylinder XZ
	Vector3D vObjectRayOrigin = Vector3D( objectRay.origin );
	double A = objectRay.direction.x*objectRay.direction.x;
    double B = 2.0 * ( objectRay.direction.x* objectRay.origin.x - 2 * m_focusxz.getValue() * objectRay.direction.z);
	double C = objectRay.origin.x * objectRay.origin.x - 4 * m_focusxz.getValue() * objectRay.origin.z;
	
	// Solve quadratic equation for _t_ values for the 1st parabolic cylinder XZ
	double t0, t1;
	...
	if( hitPoint.x < xmin || hitPoint.x > xmax || hitPoint.y > ymax || hitPoint.y < ymin ) 
		{
			if ( thit == t1 ) return false;
			if ( t1 > objectRay.maxt ) return false;
			thit = t1;
		
			// Compute possible parabolic cylinder hit position
	...
	if (hitPoint.z < (hitPoint.y*hitPoint.y/(4*m_focusyz.getValue())+z0))
		{
			intersectWithFirst = false;
			
			// Compute quadratic coefficients for the 2nd parabolic cylinder YZ
			double A = objectRay.direction.y*objectRay.direction.y;
		    double B = 2.0 * ( objectRay.direction.y* objectRay.origin.y - 2 * m_focusyz.getValue() * objectRay.direction.z);
			double C = objectRay.origin.y * objectRay.origin.y - 4 * m_focusyz.getValue() * (objectRay.origin.z - z0);
			
			// Solve quadratic equation for _t_ values  for the 2nd parabolic cylinder YZ
			if( !tgf::Quadratic( A, B, C, &t0, &t1 ) ) return false;
			
			// Compute intersection distance along ray  for the 2nd parabolic cylinder YZ
			if( t0 > objectRay.maxt || t1 < objectRay.mint ) return false;
		    thit = ( t0 > objectRay.mint )? t0 : t1 ;
		    if( thit > objectRay.maxt ) return false;
		
			// Compute parabolic cylinder hit position  for the 2nd parabolic cylinder YZ
		    hitPoint = objectRay( thit );
...
///////////////////////////////////////////////////////////////////////////////////////
	...
	Vector3D dpdu = (intersectWithFirst)? 
					Vector3D( 1.0, 0.0, x/(2.0*m_focusxz.getValue()) ) 
					: Vector3D( 1.0, 0.0, 0.0 );
	Vector3D dpdv = (intersectWithFirst)? 
					Vector3D( 0.0, 1.0, 0.0) 
					: Vector3D( 0.0, 1.0, y/(2.0*m_focusyz.getValue()) );
	
	// Compute parabaloid \dndu and \dndv
	Vector3D d2Pduu = (intersectWithFirst)? 
					Vector3D( 0.0, 0.0, 1.0/(2.0*m_focusxz.getValue()) ) 
					: Vector3D( 0.0, 0.0, 0.0 );
	Vector3D d2Pduv ( 0.0, 0.0, 0.0 );
	Vector3D d2Pdvv = (intersectWithFirst)? 
					Vector3D( 0.0, 0.0, 0.0) 
					: Vector3D( 0.0, 0.0, 1.0/(2.0*m_focusyz.getValue()) );
...
Point3D ShapeParabolicRectangle::GetPoint3D( double u, double v ) const
{
	...
	double x = xpos;
	double y = ypos;		
	double z = std::max(xpos*xpos/(4*m_focusxz.getValue()), ypos*ypos/(4*m_focusyz.getValue())+z0);
	if (xmax > ymax || m_focusyz.getValue() > m_focusxz.getValue() )
	{
		z -= z0;
	}
	if (ymax > xmax && m_focusyz.getValue() > m_focusxz.getValue() )
	{
		z += z0;
	}
...
SbVec3f ShapeParabolicRectangle::GetNormal( double u, double v ) const
{
	Trace trace( "ShapeParabolicRectangle::GetNormal", false );
	
	Point3D point = GetPoint3D( u, v );
	return SbVec3f( -point.x/2,1,0);
}
