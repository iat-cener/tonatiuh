...
void MainWindow::SetupGraphicView()
{
	...
	m_document->GetRoot()->addChild( m_vrmlBackground );
    ...
    foreach( QString fileName, fileList )
    {
        ...
        {	     
        	...
        	if( plugin->inherits("TSunShapeFactory") )
        	{
        	    ...
        	    if( !pTSunShapeFactory ) tgf::SevereError( "MainWindow::LoadPlugins: Plugin not recognized" );     	    	;
        	...
        	if( plugin->inherits("TMaterialFactory") )
        	{
        		...
        		if( !pTMaterialFactory )  tgf::SevereError( "MainWindow::LoadPlugins: Plugin not recognized" );     	    
        	...
        	if( plugin->inherits("TTrackerFactory") )
        	{
        	    ...
        	    if( !pTTrackerFactory ) tgf::SevereError( "MainWindow::LoadPlugins: Plugin not recognized" );     	    	
...
{
    Trace trace( "MainWindow::SetupActionInsertShape" );   	    
...
void MainWindow::on_actionRayTraceRun_triggered()
{
	...
	/*QProgressDialog* progressDialog = new QProgressDialog ( "Tracing rays. Please wait..." , "Cancel", 0, ( m_raysPerIteration -1 )*2, this );	
	...
	for ( long unsigned i=0; i < m_raysPerIteration; i++ )
	{
		...
		//ray = Ray( Point3D( -0.96786, 180, -8.49938 ), Vector3D( 0.000248659, -1, 0.000660587 ) );
		std::cout<<"ray" <<i<<" ---> "<<ray<<std::endl;
		tgf::TraceRay( ray, rootSeparatorInstance, *m_photonMap, *m_pRand );
  	
  	    //Update progressDiaglog when appropriate
		//if ( ( i%( m_raysPerIteration/10 ) ) == 0 ) progressDialog->setValue( i );
 	}
 	
	/*progressDialog->setValue( m_raysPerIteration-1 );
	progressDialog->setLabelText( "Balancing Photon Map. Please wait..." );
 	...
 	if( m_fraction > 0.0 || m_drawPhotons )
 	{
		...
		//progressDialog->setValue( ( m_raysPerIteration - 1 ) * 1.4 );
	...
	//progressDialog->setValue( (m_raysPerIteration - 1)*2 ); 
