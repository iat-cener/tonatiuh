...
{
	Trace trace( "SceneModel::SceneModel" );
}
...
void SceneModel::GenerateInstanceTree( InstanceNode& instanceParent )
{
	...
	if (parentNode->getTypeId().isDerivedFrom( SoBaseKit::getClassTypeId() ) )
	{
		...
		if (parentNode->getTypeId().isDerivedFrom( SoShapeKit::getClassTypeId() ) )
		{
			//Is TShapeKit node	
...
{	
	Trace trace( "SceneModel::rowCount" );
...
{
	Trace trace( "SceneModel::headerData" );
...
SoNodeKitPath* SceneModel::PathFromIndex( const QModelIndex& modelIndex ) const
{
	...
	if (!coinNode->getTypeId().isDerivedFrom( SoBaseKit::getClassTypeId() ) )
	{
		...
		return parentPath;
		//SoBaseKit* parentNode = static_cast< SoBaseKit* >( NodeFromIndex( parentIndex )->GetNode() );
		//parentNode->createPathToAnyPart()
	}
	else 
	{
