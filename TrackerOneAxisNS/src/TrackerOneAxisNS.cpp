...
#include "tgc.h"
#include "Trace.h"
#include "TrackerOneAxisNS.h"
#include "Vector3D.h"

SO_NODEENGINE_SOURCE( TrackerOneAxisNS );

void TrackerOneAxisNS::initClass()
{
	Trace trace( "TrackerOneAxisNS::initClass", false );

	SO_NODEENGINE_INIT_CLASS( TrackerOneAxisNS, TTracker, "TTracker" );

}

TrackerOneAxisNS::TrackerOneAxisNS()
{
	Trace trace( "TrackerOneAxisNS::TrackerOneAxisNS", false );

	SO_NODEENGINE_CONSTRUCTOR( TrackerOneAxisNS );
...
QString TrackerOneAxisNS::getIcon()
{
	Trace trace( "TrackerOneAxisNS::getIcon", false );

	return QString(":/icons/TrackerOneAxisNS.png");
}

void TrackerOneAxisNS::evaluate()
{
	Trace trace( "TrackerOneAxisNS::evaluate", false );
	...
	double phi = 0.0;
	if( fabs( n.z ) > tgc::Epsilon && fabs( n.y ) > tgc::Epsilon )	phi = atan2( n.z, n.y );

	SbRotation rotation( SbVec3f( 1.0, 0.0, 0.0 ), phi );
